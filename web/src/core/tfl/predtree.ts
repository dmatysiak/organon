...
export function renderPredTree(tree: PredTreeData): HTMLElement {
  ...
  for (const e of tree.treeEdges) {
    ...
    if (n1 && n2) {
      drawNodeEdge(svg, n1, n2, componentColor(n1.component));
    }
  }
  ...
  for (const e of tree.extraEdges) {
    ...
    const n2 = nodeMap.get(e.term2);
    if (n1 && n2) {
      drawNodeEdge(svg, n1, n2, "#f0a500", { dashed: true, opacity: 0.5 });
    }
  }
  ...
  for (const n of nodes) {
    drawNode(svg, n.x, n.y, n.w, n.key, {
      stroke: componentColor(n.component),
      strokeWidth: "1.5",
      tooltip: `${n.key} — category ${n.component + 1}`,
    });
  }
  ...
  drawLegend(svg, legendItems, PAD, legendY);
  ...
  summaryText.setAttribute("fill", LEGEND_COLOR);
